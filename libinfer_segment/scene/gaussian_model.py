...
import torch
import numpy as np
from libinfer_segment.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation
from torch import nn
import os
from libinfer_segment.utils.system_utils import mkdir_p
from plyfile import PlyData, PlyElement
from libinfer_segment.utils.sh_utils import RGB2SH
from simple_knn._C import distCUDA2
from libinfer_segment.utils.graphics_utils import BasicPointCloud
from libinfer_segment.utils.general_utils import strip_symmetric, build_scaling_rotation
...
class GaussianModel:
    ...
    def load_ply(self, path):
        ...
        opacities = torch.tensor(np.asarray(plydata.elements[0]["opacity"])[..., np.newaxis],
                                dtype=torch.float, device="cuda", requires_grad=False)
        
        features_dc = torch.zeros((xyz.shape[0], 3, 1), dtype=torch.float, device="cuda")
        features_dc[:, 0, 0] = torch.tensor(np.asarray(plydata.elements[0]["f_dc_0"]),
                                            dtype=torch.float, device="cuda")
        features_dc[:, 1, 0] = torch.tensor(np.asarray(plydata.elements[0]["f_dc_1"]),
                                            dtype=torch.float, device="cuda")
        features_dc[:, 2, 0] = torch.tensor(np.asarray(plydata.elements[0]["f_dc_2"]),
                                            dtype=torch.float, device="cuda")

        extra_f_names = sorted([p.name for p in plydata.elements[0].properties if p.name.startswith("f_rest_")],
                            key=lambda x: int(x.split('_')[-1]))
        assert len(extra_f_names) == 3 * (self.max_sh_degree + 1) ** 2 - 3
        features_extra = torch.zeros((xyz.shape[0], len(extra_f_names)), dtype=torch.float, device="cuda")
        for idx, attr_name in enumerate(extra_f_names):
            features_extra[:, idx] = torch.tensor(np.asarray(plydata.elements[0][attr_name]),
                                                dtype=torch.float, device="cuda")
        features_extra = features_extra.reshape((features_extra.shape[0], 3, (self.max_sh_degree + 1) ** 2 - 1))

        scale_names = sorted([p.name for p in plydata.elements[0].properties if p.name.startswith("scale_")],
                            key=lambda x: int(x.split('_')[-1]))
        scales = torch.zeros((xyz.shape[0], len(scale_names)), dtype=torch.float, device="cuda")
        for idx, attr_name in enumerate(scale_names):
            scales[:, idx] = torch.tensor(np.asarray(plydata.elements[0][attr_name]),
                                        dtype=torch.float, device="cuda")

        rot_names = sorted([p.name for p in plydata.elements[0].properties if p.name.startswith("rot")],
                        key=lambda x: int(x.split('_')[-1]))
        rots = torch.zeros((xyz.shape[0], len(rot_names)), dtype=torch.float, device="cuda")
        for idx, attr_name in enumerate(rot_names):
            rots[:, idx] = torch.tensor(np.asarray(plydata.elements[0][attr_name]),
                                        dtype=torch.float, device="cuda")

        # 将数据直接存入GPU
        self._xyz = xyz.requires_grad_(False)
        self._features_dc = features_dc.transpose(1, 2).contiguous().requires_grad_(False)
        self._features_rest = features_extra.transpose(1, 2).contiguous().requires_grad_(False)
        self._opacity = opacities.requires_grad_(False)
        self._scaling = scales.requires_grad_(False)
        self._rotation = rots.requires_grad_(False)

        self.active_sh_degree = self.max_sh_degree

        # 默认创建全1的mask
        self.segment_times = 0
        self._mask = torch.ones((self._xyz.shape[0],), dtype=torch.float, device="cuda").requires_grad_(False)
    ...
    @torch.no_grad()
    def edit(self, obj_move = np.array([0,0,0]), mask=None):
        assert mask is not None
            # mask = (self._mask > 0)
        mask = mask.squeeze()
        # assert mask.shape[0] == self._xyz.shape[0]
        ...
        if self.old_xyz == []:
            self.old_xyz.append(self._xyz.detach().clone())
            self.old_mask.append(self._mask.detach().clone())
            self.old_features_dc.append(self._features_dc.detach().clone())
            self.old_features_rest.append(self._features_rest.detach().clone())
            self.old_opacity.append(self._opacity.detach().clone())
            self.old_scaling.append(self._scaling.detach().clone())
            self.old_rotation.append(self._rotation.detach().clone())
        else:
        # self._xyz = self._xyz[mask]
        # self._features_dc = self._features_dc[mask]
        # self._features_rest = self._features_rest[mask]
        ...
        # self._scaling = self._scaling[mask]
        # self._rotation = self._rotation[mask]
            print(self.old_xyz[0][mask][0].detach().cpu().numpy(), self._xyz[mask][0].detach().cpu().clone().numpy())
            self._xyz = self.old_xyz[0].clone()
            self._mask = self.old_mask[0]
            self._features_dc = self.old_features_dc[0].clone()
            self._features_rest = self.old_features_rest[0].clone()
            self._opacity = self.old_opacity[0].clone()
            self._scaling = self.old_scaling[0].clone()
            self._rotation = self.old_rotation[0].clone()
            
        self._xyz[mask] = self._xyz[mask]+torch.tensor(obj_move, dtype=torch.float, device="cuda")
    ...
    def roll_back(self):
        try:
            self._xyz = torch.tensor(self.old_xyz.pop(),)
    ...
    def concat_gaussians(self, another_gaussian):
        ...
        start_idx = self._xyz.shape[0]-1
        end_idx = self._xyz.shape[0]+new_xyz.shape[0]-1
        ...
        self._mask = torch.cat((self._mask, torch.ones((self._xyz.shape[0],), dtype=torch.float, device="cuda")),  dim=0)
