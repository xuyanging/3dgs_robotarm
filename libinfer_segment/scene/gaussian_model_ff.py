...
class FeatureGaussianModel:
    ...
    def edit(self, obj_move=np.array([0,0,0]), mask=None):
        ...
        if self.old_xyz == []:
            ...
            self.old_point_features.append(self._point_features.detach().clone())
        ...
        # self._rotation = self._rotation[mask]
            ...
            self._point_features = self.old_point_features[0].clone()
    ...
    def load_ply(self, path):
        ...
        # 将 ply 数据直接转换为 GPU 上的 Tensor
        xyz = torch.tensor(np.stack((plydata.elements[0]["x"],
                                    plydata.elements[0]["y"],
                                    plydata.elements[0]["z"]), axis=1), dtype=torch.float, device="cuda")

        opacities = torch.tensor(np.asarray(plydata.elements[0]["opacity"])[..., np.newaxis], dtype=torch.float, device="cuda")

        f_names = sorted([p.name for p in plydata.elements[0].properties if p.name.startswith("f_")], 
                        key=lambda x: int(x.split('_')[-1]))
        assert len(f_names) == self.feature_dim

        features_extra = torch.tensor(np.column_stack([np.asarray(plydata.elements[0][name]) for name in f_names]), 
                                    dtype=torch.float, device="cuda")

        scale_names = sorted([p.name for p in plydata.elements[0].properties if p.name.startswith("scale_")], 
                            key=lambda x: int(x.split('_')[-1]))
        scales = torch.tensor(np.column_stack([np.asarray(plydata.elements[0][name]) for name in scale_names]), 
                            dtype=torch.float, device="cuda")

        rot_names = sorted([p.name for p in plydata.elements[0].properties if p.name.startswith("rot")], 
                        key=lambda x: int(x.split('_')[-1]))
        rots = torch.tensor(np.column_stack([np.asarray(plydata.elements[0][name]) for name in rot_names]), 
                            dtype=torch.float, device="cuda")

        # 直接将所有数据放到显存中
        self._xyz = xyz.requires_grad_(False)
        self._point_features = features_extra.requires_grad_(False)
