...
import torch
import numpy as np
from libinfer_3dgs.utils.general_utils import inverse_sigmoid, get_expon_lr_func, build_rotation
from torch import nn
from plyfile import PlyData, PlyElement
from libinfer_3dgs.utils.system_utils import mkdir_p
from libinfer_3dgs.utils.sh_utils import RGB2SH
from simple_knn._C import distCUDA2
from libinfer_3dgs.utils.graphics_utils import BasicPointCloud
from libinfer_3dgs.utils.general_utils import strip_symmetric, build_scaling_rotation
...
class GaussianModel:
    ...
    #         features_extra[:, idx] = torch.Tensor(np.asarray(plydata.elements[0][attr_name])).cuda()
    ...
    #     self._features_rest = nn.Parameter(features_extra.transpose(1, 2).contiguous().requires_grad_(False))
    ...
    def load_ply(self, path):
        ...
        scale_names = sorted([p.name for p in plydata.elements[0].properties if p.name.startswith("scale_")], 
                            key=lambda x: int(x.split('_')[-1]))
        scales = np.vstack([plydata.elements[0][attr_name] for attr_name in scale_names]).T

        # 获取rotation名称并排序
        rot_names = sorted([p.name for p in plydata.elements[0].properties if p.name.startswith("rot")], 
                        key=lambda x: int(x.split('_')[-1]))
        rots = np.vstack([plydata.elements[0][attr_name] for attr_name in rot_names]).T

        # 使用PyTorch直接将数据转移到GPU，避免不必要的拷贝
        features_extra = torch.zeros((xyz.shape[0], len(extra_f_names)), dtype=torch.float, device="cuda")
        for idx, attr_name in enumerate(extra_f_names):
            features_extra[:, idx] = torch.from_numpy(np.asarray(plydata.elements[0][attr_name])).cuda()

        # Reshape features_extra
        features_extra = features_extra.reshape((features_extra.shape[0], 3, (self.max_sh_degree + 1) ** 2 - 1))

        # 使用nn.Parameter并在初始化时直接将数据转移到GPU，避免重复分配和拷贝
        self._xyz = torch.tensor(xyz, dtype=torch.float, device="cuda", requires_grad=False)
        self._features_dc = torch.tensor(features_dc, dtype=torch.float, device="cuda", requires_grad=False).transpose(1, 2).contiguous()
        self._features_rest = features_extra.transpose(1, 2).contiguous()
        self._opacity = torch.tensor(opacities, dtype=torch.float, device="cuda", requires_grad=False)
        self._scaling = torch.tensor(scales, dtype=torch.float, device="cuda", requires_grad=False)
        self._rotation = torch.tensor(rots, dtype=torch.float, device="cuda", requires_grad=False)
