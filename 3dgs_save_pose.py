...
def main():
    ...
    # Define the directory to save images and parameters
    ...
    def update_image():
        ...
        with torch.no_grad():
            # Get the camera's Euler angles and position
            yaw, pitch, roll = key_input_handler.get_euler_angles()
            x, y, z = key_input_handler.get_position()
            
            # Compute the view matrix
            view_matrix = p.computeViewMatrixFromYawPitchRoll(cameraTargetPosition=[x, y, z], distance=5, yaw=yaw, pitch=pitch, roll=roll, upAxisIndex=2)
            ...
            rendering = render(custom_cam, gaussians, pipeline, white_background)["render"]
            img_buffer_white = rendering.permute(1, 2, 0).cpu().numpy()
            rendering = render(custom_cam, gaussians, pipeline, black_background)["render"]
            img_buffer_black = rendering.permute(1, 2, 0).cpu().numpy()
            mask = ~np.array((img_buffer_white - img_buffer_black)>0.5, np.bool_)
            # Display the image and FPS on the GUI
            dpg.set_value("rendered_image", img_buffer_black.reshape(-1))
            dpg.set_value("euler_angles_text", f'FPS: {fps:.2f}')
            
            # Save the image and parameters
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S%f")
            image_filename = os.path.join(save_dir, f"image_{timestamp}.png")
            params_filename = os.path.join(save_dir, f"params_{timestamp}.txt")
            mask_filename = os.path.join(save_dir, f"mask_{timestamp}.png")
            # from PIL import Image
            # Image.fromarray((img_buffer_black * 255).astype(np.uint8)).save(image_filename)
            # Image.fromarray((mask * 255).astype(np.uint8)).save(mask_filename)
            # Save the intrinsic and extrinsic parameters
            # with open(params_filename, "w") as f:
            #     f.write(f"Yaw: {yaw}\nPitch: {pitch}\nRoll: {roll}\n")
            #     f.write(f"Position: x={x}, y={y}, z={z}\n")
            #     f.write("View Matrix:\n")
            #     f.write(np.array2string(view_matrix, separator=', ') + "\n")
            #     f.write("Projection Matrix:\n")
            #     f.write(np.array2string(projection_matrix_torch.cpu().numpy(), separator=', ') + "\n")
